# Промпт-шаблоны — AI Media Production Pipeline V3.2
...
END FRAME: [точное визуальное состояние: поза (not mid-action), выражение лица, положение камеры (stable), расположение объектов, свет. No motion blur на ключевых объектах]. Используется как start_image следующего сегмента.
...
END FRAME: [точное состояние обоих персонажей (not mid-action), камеры (stable), фона. No motion blur.]
...
### NANO Mode (3 строки — для самых упрямых быстрых моделей)
```text
...
3. Для видео/сюжета главным является Shot Board / Storyboard (≤ 6 panels / 15c для Seedance).
4. Keyframes — опциональные anchors, не обязательны после storyboard.
5. Panels ≠ Beats: Panels = стадии Shot Board (≤6/15c). Beats = события в таймлайне (≤4/5c).
6. Для сложных видео — Segment Storyboards + несколько генераций.
7. Каждый этап генерации: Workflow → Stage Gate → Upload Map → Reference Map → Prompt → QC.
8. Созданный board обязан использоваться дальше через Reference Map.
9. Экраны устройств с composited контентом: экран = #00FF00. VFX NOTE в Shot Board.
10. Multi-segment: END FRAME обязателен (not mid-action, camera stable, no motion blur).
11. 2+ персонажа: фиксировать нумерацию @image и не менять между сегментами.
12. Level 1 = motion test опционален. Level 2+ = обязателен.
13. Остановись после первого плана и жди подтверждения.
```
